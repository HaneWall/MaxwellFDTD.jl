...
!***********************************************************************
!
!     Program author: Jamesina J. Simpson, Associate Professor
...
!***********************************************************************
 ...
 !  Specify the PEC Plate Boundaries and the Source/Recording Points
    INTEGER, PARAMETER ::                                    &
       istart = 25, iend = 25, jstart = 25,   &
       jend = 25, kstart = 25, kend = 25,      &
       isource = istart, jsource = jstart, ksource = kstart,     &
  !     irecv1 = iend, jrecv1 = jend+1, krecv1 = kend  ! Ey at probe point
       irecv1 = 25, jrecv1 = 25, krecv1 = 25  ! Ey at probe point
 !  ..................................
 ...
 !     TM components
    REAL( kind = 8),DIMENSION(Imax, Jmax, Kmax-1)  ::                      &
       Ez, CA, CB, sig, eps
 
    REAL( kind = 8),DIMENSION(Imax-1, Jmax, Kmax-1)  ::                      &
       Hy
 
    REAL( kind = 8),DIMENSION(Imax,Jmax-1, Kmax-1)  ::                      &
       Hx
 ...
 !     TE components
    REAL( kind = 8),DIMENSION(Imax-1, Jmax-1, Kmax)  ::                      &
       Hz
 
    REAL( kind = 8),DIMENSION(Imax-1, Jmax, Kmax)  ::                      &
       Ex
 
    REAL( kind = 8),DIMENSION(Imax,Jmax-1, Kmax)  ::                      &
       Ey
 
 !  PML
    REAL( kind = 8) ,DIMENSION(nxPML_1,Jmax,Kmax) ::                       &
       psi_Ezx_1
 
    REAL( kind = 8) ,DIMENSION(nxPML_2,Jmax,Kmax) ::                       &
       psi_Ezx_2
 
    REAL( kind = 8) ,DIMENSION(nxPML_1-1,Jmax,Kmax) ::                       &
       psi_Hyx_1
 
    REAL( kind = 8) ,DIMENSION(nxPML_2-1,Jmax,Kmax) ::                       &
       psi_Hyx_2
 
    REAL( kind = 8) ,DIMENSION(Imax,nyPML_1,Kmax) ::                       &
       psi_Ezy_1                               
 
    REAL( kind = 8) ,DIMENSION(Imax,nyPML_2,Kmax) ::                       &
       psi_Ezy_2
 
    REAL( kind = 8) ,DIMENSION(Imax,nyPML_1-1,Kmax) ::                       &
       psi_Hxy_1                               
 
    REAL( kind = 8) ,DIMENSION(Imax,nyPML_2-1,Kmax) ::                       &
       psi_Hxy_2
 
    REAL( kind = 8) ,DIMENSION(Imax,Jmax-1,nzPML_1-1) ::                       &
       psi_Hxz_1
 
    REAL( kind = 8) ,DIMENSION(Imax,Jmax-1,nzPML_2-1) ::                       &
       psi_Hxz_2
 
    REAL( kind = 8) ,DIMENSION(Imax-1,Jmax,nzPML_1-1) ::                       &
       psi_Hyz_1
 
    REAL( kind = 8) ,DIMENSION(Imax-1,Jmax,nzPML_2-1) ::                       &
       psi_Hyz_2
 
    REAL( kind = 8) ,DIMENSION(Imax-1,Jmax,nzPML_1) ::                       &
       psi_Exz_1
 
    REAL( kind = 8) ,DIMENSION(Imax-1,Jmax,nzPML_2) ::                       &
       psi_Exz_2
 
    REAL( kind = 8) ,DIMENSION(Imax,Jmax-1,nzPML_1) ::                       &
       psi_Eyz_1
 
    REAL( kind = 8) ,DIMENSION(Imax,Jmax-1,nzPML_2) ::                       &
       psi_Eyz_2
 
    REAL( kind = 8) ,DIMENSION(nxPML_1-1,Jmax-1,Kmax) ::                       &
       psi_Hzx_1
    REAL( kind = 8) ,DIMENSION(nxPML_1,Jmax-1,Kmax) ::                       &
       psi_Eyx_1
 
    REAL( kind = 8) ,DIMENSION(nxPML_2-1,Jmax-1,Kmax) ::                       &
       psi_Hzx_2
    REAL( kind = 8) ,DIMENSION(nxPML_2,Jmax-1,Kmax) ::                       &
       psi_Eyx_2
 
    REAL( kind = 8) ,DIMENSION(Imax-1,nyPML_1-1,Kmax) ::                       &
       psi_Hzy_1                               
    REAL( kind = 8) ,DIMENSION(Imax-1,nyPML_1,Kmax) ::                       &
       psi_Exy_1                               
 
    REAL( kind = 8) ,DIMENSION(Imax-1,nyPML_2-1,Kmax) ::                       &
       psi_Hzy_2
    REAL( kind = 8) ,DIMENSION(Imax-1,nyPML_2,Kmax) ::                       &
       psi_Exy_2
 
    REAL( kind = 8) ,DIMENSION(nxPML_1) ::                       &
       be_x_1, ce_x_1, alphae_x_PML_1, sige_x_PML_1, kappae_x_PML_1
    REAL( kind = 8) ,DIMENSION(nxPML_1-1) ::                       &
       bh_x_1, ch_x_1, alphah_x_PML_1, sigh_x_PML_1, kappah_x_PML_1
 
    REAL( kind = 8) ,DIMENSION(nxPML_2) ::                       &
       be_x_2, ce_x_2, alphae_x_PML_2, sige_x_PML_2, kappae_x_PML_2
    REAL( kind = 8) ,DIMENSION(nxPML_2-1) ::                       &
       bh_x_2, ch_x_2, alphah_x_PML_2, sigh_x_PML_2, kappah_x_PML_2
 
    REAL( kind = 8) ,DIMENSION(nyPML_1) ::                       &
       be_y_1, ce_y_1, alphae_y_PML_1, sige_y_PML_1, kappae_y_PML_1
    REAL( kind = 8) ,DIMENSION(nyPML_1-1) ::                       &
       bh_y_1, ch_y_1, alphah_y_PML_1, sigh_y_PML_1, kappah_y_PML_1
 
    REAL( kind = 8) ,DIMENSION(nyPML_2) ::                       &
       be_y_2, ce_y_2, alphae_y_PML_2, sige_y_PML_2, kappae_y_PML_2
    REAL( kind = 8) ,DIMENSION(nyPML_2-1) ::                       &
       bh_y_2, ch_y_2, alphah_y_PML_2, sigh_y_PML_2, kappah_y_PML_2
 
    REAL( kind = 8) ,DIMENSION(nzPML_1) ::                       &
       be_z_1, ce_z_1, alphae_z_PML_1, sige_z_PML_1, kappae_z_PML_1
    REAL( kind = 8) ,DIMENSION(nzPML_1-1) ::                       &
       bh_z_1, ch_z_1, alphah_z_PML_1, sigh_z_PML_1, kappah_z_PML_1
 
    REAL( kind = 8) ,DIMENSION(nzPML_2) ::                       &
       be_z_2, ce_z_2, alphae_z_PML_2, sige_z_PML_2, kappae_z_PML_2
    REAL( kind = 8) ,DIMENSION(nzPML_2-1) ::                       &
       bh_z_2, ch_z_2, alphah_z_PML_2, sigh_z_PML_2, kappah_z_PML_2
 
 !     denominators for the update equations
    REAL( kind = 8),DIMENSION(Imax-1)  ::                      &
       den_ex, den_hx
 
    REAL( kind = 8),DIMENSION(Jmax-1)  ::                      &
       den_ey, den_hy
 
    REAL( kind = 8),DIMENSION(Kmax-1)  ::                      &
       den_ez, den_hz
