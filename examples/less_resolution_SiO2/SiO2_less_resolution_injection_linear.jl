...
signal_injection_theory[center_help_L_low:center_help_L_high] = window .* P_eff
...
lines!(ω, abs.((FT_P_1)./(padded_L)) .+ 0.01, color=:blue, label=L"P_{1} (\omega) = FT[\chi^{(1)} t_{lin} E](\omega)")
lines!(ω, abs.(r_theory .* (FT_P_1)./(padded_L)) .+ 0.01, color=:blue, label=L"r_{lin} P_{1} (\omega)", linestyle=:dash)
...
save("linear_injection_reflection.pdf", fig_relative_amplitudes)