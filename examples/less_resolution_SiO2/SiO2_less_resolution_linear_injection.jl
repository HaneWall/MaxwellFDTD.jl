...
#     [0.0, 20.0],
#     [0.0, 25.0],
#     ["J_{Kerr}", "J_{Injection}"],
#     true,
#     "Tangent_injection_kerr.pdf")
...
ax = Axis(fig_harm[1, 1], title="Normed Injection amplitude and degeneracies", subtitle="Tangent ADK, Kerr and Injection")
...
ax2 = Axis(fig_harm[2, 1], title="log10 normed Injection amplitude and Degens", subtitle="Tangent ADK, Kerr and Injection", xlabel = L"ω / ω_{pump}")
...
scatter!(h_injection, log10.(normed_degen_injection), color=:red, label="Normed permutations")
ylims!(ax2, [-5.0, 0.0])
xlims!(ax2, [0.0, 15.0])
...
j_kerr_measured = d2.J_Bound[30000:40000]
j_kerr_amplitude_spectrum = zeros(Float64, padded_L)
j_kerr_amplitude_spectrum[center_help_L_low:center_help_L_high] = window .* j_kerr_measured
FT_j_kerr = fftshift(fft(j_kerr_amplitude_spectrum))
j_injecton_measured = d2.J_Tunnel[30000:40000]
j_injection_amplitude_spectrum = zeros(Float64, padded_L)
j_injection_amplitude_spectrum[center_help_L_low:center_help_L_high] = window .* j_injecton_measured
FT_j_injection = fftshift(fft(j_injection_amplitude_spectrum))
...
lines!(ω, log10.((abs.(ω .*  FT_reflection))./padded_L), color=:red, label=L"Kerr + Injection, \log_{10}\mid \omega E_z \mid")
lines!(ω, log10.((abs.(FT_j))./padded_L) , color=:red, label=L"Kerr + Injection, \log_{10} \mid J_z \mid")
lines!(ω, log10.((abs.(FT_j_kerr))./padded_L) , color=:red, linestyle=:dashdot, label=L"Kerr, \log_{10}\mid J_z \mid")
lines!(ω, log10.((abs.(FT_j_injection))./padded_L) , color=:red, linestyle=:dot, label=L"only Injection, \log_{10}\mid J_z \mid")
lines!(ω, log10.((abs.(ω .*  FT_reflection_one_cell))./padded_L), color=:black,  label=L"Injection, \log_{10}\mid \omega E_z \mid")

lines!(ω, log10.((abs.(FT_j_one_cell))./padded_L) , color=:black, label=L"Injection, \log_{10}\mid J_z \mid")

ylims!(ax1, [0.0, 15.0])
xlims!(ax1, [0.0, 15.0])
axislegend(ax1, position=:rt)
fig_refl
#save("reflection_amplitude_injection_kerr_Tangent_ADK_2.pdf", fig_refl)

## E-Amplitudenverhältnisse zwischen Reflektion und erster Zelle

# E_measured_bulk = d2.Ez[30000:40000]
# #E_measured_bulk = d3.Ez[37000:47000]
...
# ax1 =Axis(fig_relative_amplitudes[1, 1], xlabel=L"\omega / \omega_{pump}", title="amplitudes", subtitle="Tangent ADK, a= $eff_nl") 
# lines!(ω, log10.(abs.(FT_E_bulk)), color=:blue)
# lines!(ω, log10.(abs.(FT_reflection)), color=:red)
# ylims!(ax1, [4.0, 16.0])
# xlims!(ax1, [0.0, 15.0])
# ax2 =Axis(fig_relative_amplitudes[2, 1], xlabel=L"\omega / \omega_{pump}", title="amplitudes", subtitle="Tangent ADK, a= $eff_nl") 
# lines!(ω, abs.(FT_E_bulk)./abs.(FT_reflection))
# ylims!(ax2, [0.9, 5.1])
# xlims!(ax2, [0.0, 15.0])
