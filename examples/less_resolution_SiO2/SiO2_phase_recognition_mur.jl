...
m1 = LorentzMedium1D(g, CartesianIndices((3000:5980,)), 1.0, γ_lorentz, ω_0, χ_1, χ_2, χ_3)
m2 = DrudeMedium1D(g, CartesianIndices((3000:5980,)), γ_plasma, ρ_mol_density)
m3 = TunnelMedium1D(g, CartesianIndices((3000:5980,)), E_gap, ρ_mol_density)
...
d5 = PointDetector(CartesianIndex((1000,)), 1, g.MaxTime)
detectors = [d2, d4, d5]
...
L = length(t)
padded_L = 2*nextpow(2, L)
t_max = g.Δt + padded_L * g.Δt
t_padded = g.Δt:g.Δt:t_max 
t_padded_int = 1.:1.:padded_L
δt = abs(t[2]-t[1])
δf = 1/δt
...
kth_harmonic_idx = [argmin(abs.(ω .- k)) for k in ks]
...
z_bulk, ϕ_E_tr_2 = analytic_signal(E_amplitude_spectrum, Array(t_padded), t_ref)
ϕ_E_tr_harms = [real(ϕ_E_tr_2[idx]) for idx in kth_harmonic_idx]
...
z_inj, ϕ_j_inj = analytic_signal(signal_j_inj, Array(t_padded), t_ref)
ϕ_j_inj_harms = [real(ϕ_j_inj[idx]) for idx in kth_harmonic_idx]


signal_j_brunel = zeros(Float64, padded_L)
signal_j_brunel[window_idx] = window .* j_brunel
z_brunel, ϕ_j_brunel = analytic_signal(signal_j_brunel, Array(t_padded), t_ref)
ϕ_j_brunel_harms = [real(ϕ_j_brunel[idx]) for idx in kth_harmonic_idx]

signal_j_bound = zeros(Float64, padded_L)
signal_j_bound[window_idx] = window .* j_kerr
z_bound, ϕ_j_bound = analytic_signal(signal_j_bound, Array(t_padded), t_ref)
ϕ_j_bound_harms = [real(ϕ_j_bound[idx]) for idx in kth_harmonic_idx]
...
z_r, ϕ_E_r_2 = analytic_signal(E_amplitude_spectrum_refl, Array(t_padded), t_ref)
ϕ_E_r_harms = [real(ϕ_E_r_2[idx]) for idx in kth_harmonic_idx]
...
function get_phase_spectrum(ω::Array{Float64}, spec::Array{Complex{Float64}}, harmonics::Array{Float64}, padded_L::Int64)
    ...
    for (idx, k) in enumerate(harmonics)
        ...
        # isolate spectra
        FT_k = isolate_around_freq2(ω, spec, k, 2., true)
        # ϕ = angle.(FT_k)
...
function get_z_and_phase_spectrum(ω::Array{Float64}, spec::Array{Complex{Float64}}, harmonics::Array{Float64}, padded_L::Int64)
    L = size(harmonics, 1)
    ϕs = zeros(Float64, (L, padded_L))
    zs = zeros(Complex{Float64}, (L, padded_L))
    for (idx, k) in enumerate(harmonics)
        kth_harmonic_idx = argmin(abs.(ω .- k))
        # isolate spectra
        FT_k = isolate_around_freq2(ω, spec, k, 2., true)
        ...
        t = Array(1.:1.:padded_L)
        t_ref = Float64(argmax(abs.(k_t)))
        z, ϕ = analytic_signal(real(k_t), t, t_ref)
        ϕs[idx] = ϕ[kth_harmonic_idx]
        zs[idx, :] = z[:]
    end        
    return zs, ϕs
...
# fig = Makie.Figure(resolution = (600, 600), font="CMU Serif")
# ax1 = Axis(fig[1, 1])
# scatterlines!(ks, unwrap(ϕ_Brunel) .- unwrap(ϕ_Inj), label=L"\phi_{Brunel}")
# scatterlines!(ks, unwrap(ϕ_E_r),label=L"\phi_{Er}")
# scatterlines!(ks, unwrap(ϕ_E_tr),label=L"\phi_{Etr}")
# #scatterlines!(ks, ϕ_Kerr, label=L"\phi_{Kerr}")
# scatterlines!(ks, unwrap(ϕ_Inj),label=L"\phi_{Inj}")
...
#d = ax.scatter(k_phase_harms, r, label="k1") 
e = ax.scatter(ϕ_Brunel, r, label=L"J_{Brunel}", marker="s")
f = ax.scatter(ϕ_E_r, r, label=L"E_{Reflection}")
#i = ax.scatter(ϕ_E_r_2, r, label=L"E_{Reflection, 2}")
...
ax[:grid](true)
savefig("Phasen_gamma_1e15_mur2_tangent.pdf")
...
FT_E_k = isolate_around_freq2(Array(ω), FT_E_bulk, k, 1.8, false)
...
z_j_brunel, ϕ_j_brunel = analytic_signal2(real(j_brunel_k_t), t_help, t_ref)
...
ax1 = Axis(fig[1, 1])
lines!(ax1, real.(E_k_t)./maximum(abs.(E_k_t)), label=L"E_{Bulk}")
lines!(ax1, real.(Er_k_t)./maximum(abs.(Er_k_t)), label=L"E_{Refl}")
...
inj_harms, phi_inj = get_z_and_phase_spectrum(Array(ω), FT_J_inj, ks, padded_L)
fig = Makie.Figure(resolution = (800, 800), font="CMU Serif")
ax = Axis(fig[1, 1])
lines!(ax, real.(inj_harms[2, :])./maximum(abs.(inj_harms[1, :])))
# lines!(ax, abs.(inj_harms[2, :])./maximum(abs.(inj_harms[2, :])))
# lines!(ax, abs.(inj_harms[3, :])./maximum(abs.(inj_harms[3, :])))
xlims!(ax, [34000,36000])
fig
