...
lines!(ω, (abs.(FT_E_vacuum)./padded_L) .+0.01, color=:black, label=L"E_{vac}")
# ylims!(ax1, [0.0, 9.0])
xlims!(ax1, [0.0, 2.0])
axislegend(ax1, position=:lt)
ax2 =Axis(fig_relative_amplitudes[2, 1], xlabel=L"\omega / \omega_{pump}", title="Fresnelkoeffizienten - Amplitudenrelationen")
lines!(ω, (abs.(FT_E_bulk)./padded_L)./(abs.(FT_E_vacuum)./padded_L), color=:red, label="t_s - FDTD")
lines!(ω, abs.(t_theory), color=:gray, linestyle=:dash, label="t_s - Theory")
lines!(ω, abs.(FT_E_bulk_refl)./abs.(FT_E_vacuum), color=:blue, label="r_s - FDTD")
lines!(ω, abs.(r_theory), color=:black, linestyle=:dash, label="r_s - Theory")
xlims!(ax2, [0.5, 1.5])
ylims!(ax2, [0., 1.0])
axislegend(ax2, position=:lc)
fig_relative_amplitudes
save("linear_fresnel.pdf", fig_relative_amplitudes)