...
lines!(ω, 0.6*10e-26.*(abs.((FT_J)./(padded_L)) .+ 0.01)./abs.(ω), color=:black, label=L"J_{eff} = C \cdot FT[\chi^{(Brunel, stat)} E^{13}](\omega) / \omega")
...
save("reflection_amplitude_brunel_tangent_stat_theory.pdf", fig_relative_amplitudes)

fig_time = Figure(resolution=(800, 800), font="CMU Serif")
ax1 = Axis(fig_time[1, 1], xlabel="Timesteps", ylabel="Amplitude")
lines!(t_arr, E_measured_bulk, color=:red, label=L"E_{tr}")
lines!(t_arr, E_measured_reflection, color=:red, linestyle=:dash, label=L"E_{re}")
axislegend(ax1, position=:rt)
ax2 = Axis(fig_time[2, 1], xlabel="Timesteps", ylabel="Amplitude") 
lines!(t_arr, j_eff, color=:black, label=L"J_{eff}")
axislegend(ax2, position=:rt)
fig_time
save("time_amplitudes_brunel.pdf", fig_time)