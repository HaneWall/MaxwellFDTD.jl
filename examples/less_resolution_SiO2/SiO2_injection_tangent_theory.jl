...
function χ_injection_dyn(Γ̂, Ê, E_arr, E_gap, n_0, a)
    return E_gap * q_0 * n_0 * Γ̂ ./ ( Ê^(a) .* abs.(E_arr).^2)
end
...
#χ_injection = χ_injection_dyn(Γ̂, Ê, E_arr, E_gap*q_0, ρ_mol_density, eff_nl)
...
lines!(ω, 0.6*10e-6.*(abs.((FT_J)./(padded_L)) .+ 0.01)./abs.(ω), color=:black, label=L"J_{eff} = C \cdot FT[\chi^{(Injection)} E^{13}](\omega) / \omega")
...
save("linear_only_injection_reflection.pdf", fig_relative_amplitudes)